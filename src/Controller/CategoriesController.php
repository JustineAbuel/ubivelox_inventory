...
class CategoriesController extends AppController
{
    ...
    public function index()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            if ($_FILES["file"]["size"] > 0) {
                ...
                $requiredHeaders = ['*Category Name', 'Description'];
                ...
                if ($foundHeaders !== $requiredHeaders) {
                    ...
                    $this->Common->dblogger([
                        //change depending on action
                        'message' => 'Uploaded CSV is not the correct Categories CSV template. Please, try again.',
                        'request' => $this->request,
                        'status' => 'error',
                    ]);
                ...
                while ($data = fgetcsv($file)) {
                    ...
                    if ($data[0] == '') {
                        ...
                        $this->Common->dblogger([
                            //change depending on action
                            'message' => 'Mass upload[Categories] - Could not save row record',
                            'request' => $this->request,
                            'status' => 'error'
                        ]);
                        ...
                        $this->Common->dblogger([
                            //change depending on action
                            'message' => 'Mass upload[Items] - Category already exists',
                            'request' => $this->request,
                            'status' => 'error'
                        ]);
                ...
                if ($counter > 0) {
                    ...
                    $this->Flash->error(__('Category CSV has been uploaded. {0} category could not be saved.', $errorCounter));
                } elseif ($itemExists > 0) {
                    $this->Flash->error(__('Category CSV has been uploaded. {0} category already exists.', $itemExists));
                } else {
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $category = $this->Categories->patchEntity($category, $this->request->getData());
            // dd($category);  
            // if ($this->checkifrecordexists($category->category_name)) {
    ...
    public function checkifrecordexists($category_name)
    {

        if ($this->Categories->findByCategoryName($category_name)->count() > 0) {
            // $this->Flash->error(__('The category already exists. Please, try again.'));
            return true;
        }
    }
